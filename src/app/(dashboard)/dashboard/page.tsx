...
export default async function DashboardHomePage() {
  ...
  const professors_tutoring_sessions:{ts_count: bigint}[] = await prisma.$queryRaw`
  ...
  const professors_tutoring_session_engagement:{ts_attendee_count:number}[] = await prisma.$queryRaw`
  ...
  const num_professors_tutoring_sessions: number = Number(professors_tutoring_sessions[0].ts_count);
  const num_professors_tutoring_engagement: number = Number(professors_tutoring_session_engagement[0].ts_attendee_count);
  ...
  const professors_tutors_month_prior:{ta_count: bigint}[] = await prisma.$queryRaw`
  SELECT COUNT(*) as TA_Count
  FROM "ta_schedules" as ts
  JOIN "users" as u_ta ON u_ta."User_ID" = ts."TA_ID"
  JOIN "classes" c ON ts."Class_Assigned" = c."Class_ID"
  ...
  const tutoring_sessions_week_prior:{ts_count: bigint}[] = await prisma.$queryRaw`
  SELECT COUNT(*) as ts_count
  FROM "tutoring_sessions" as ts
  JOIN "classes" c ON ts."Class_Tutored" = c."Class_ID"
  JOIN "professor_classes" pc ON c."Class_ID" = pc."Class_ID"
  ...
  const session_engagement_semester_prior:{ts_attendee_count:number}[] = await prisma.$queryRaw`
  SELECT SUM("Students_Tutored") as ts_attendee_count
  FROM "tutoring_sessions" as ts
  JOIN "classes" c ON ts."Class_Tutored" = c."Class_ID"
  JOIN "professor_classes" pc ON c."Class_ID" = pc."Class_ID"
  ...
  const num_tutors_month_prior: number = Number(professors_tutors_month_prior[0].ta_count);
  const num_sessions_week_prior: number = Number(tutoring_sessions_week_prior[0].ts_count);
  const num_engagement_semester: number = Number(session_engagement_semester_prior[0].ts_attendee_count);
      ...
      <div className="w-full">
        <div className="flex flex-wrap md:flex-nowrap gap-2 md:gap-4">
          <AnalyticsCard
            ...
            filterData={num_professors_tutoring_engagement - num_engagement_semester}
            range={"semester"}
          />
          <AnalyticsCard
            title="Active TAs"
          ...
          <AnalyticsCard
            ...
            filterData={num_professors_tutoring_sessions - num_sessions_week_prior}
            range={"week"}
          />
        </div>
      </div>
